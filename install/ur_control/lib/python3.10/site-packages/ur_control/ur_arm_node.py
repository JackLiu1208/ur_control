...
# 自己的座標系下量測。若工具還有旋轉偏移（不是單純沿 Z 伸出去），現在的實作不
# 支援，需要另外擴充。move_pose()/move_pose_linear() 的 position_xyz 與
# get_current_tcp_pose() 回傳的都是「這個 TCP 點」的座標，不是法蘭面本身——
# compute_ik_analytic()/get_current_tcp_pose_analytic() 也是，IK/FK 本身
# （analytic_ik.py）完全不知道 TCP offset 這件事，只認 tool0，offset 是這裡、
# 呼叫 IK/FK 前後另外疊加的一個固定平移，換工具只要換這個值，不需要動 IK。
#
# 兩組常用預設：換工具（例如裝/拆夾爪）時改 tcp_offset_xyz=<這裡> 就好。
TCP_OFFSET_BARE_FLANGE = (0.0, 0.0, 0.0)        # 沒有裝任何工具，TCP = tool0 法蘭面本身

# 這個值不是「25cm 夾爪所以填 0.25」這種估算——是實測的：夾爪 finger_joint 在
# xacro 設定的 gripper_closed_position（0.695，全閉）時，直接查 TF
# tool0 -> left_inner_finger_pad / right_inner_finger_pad 量出來的（兩指中點，
# x/y 分量互相抵消趨近 0），跟原本估的 0.25 差了快 4cm——RViz 裡 marker 跟實際
# 夾爪末端對不上，就是這個誤差造成的（3.8cm 的固定偏移在某些相機角度看起來還好、
# 某些角度看起來差很多，不是真的忽好忽壞，是同一個誤差、視角不同而已）。
# 夾爪指尖位置隨開合angle 會有些微變動（四連桿機構，不是純直線位移），這個值是
# 全閉狀態下量的——如果你的夾取任務主要用全開或半開狀態，建議在那個狀態下重新
# 用同樣方法（lookup_transform("tool0", "left_inner_finger_pad")）量一次。
TCP_OFFSET_ROBOTIQ_2F140 = (0.0, 0.0, 0.2117)   # 25cm 長的 Robotiq 2F-140 夾爪，全閉狀態實測

DEFAULT_TCP_OFFSET_XYZ = TCP_OFFSET_ROBOTIQ_2F140
